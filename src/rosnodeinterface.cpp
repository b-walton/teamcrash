...
// C/C++ source code generated on : Tue Nov 28 10:53:32 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"teamcrash\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(teamcrash_M, (NULL));
        teamcrash_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(teamcrash_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(teamcrash_M)
                == (NULL)) || rtmGetStopRequested(teamcrash_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(teamcrash_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
